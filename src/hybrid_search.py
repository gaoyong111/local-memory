"""local-memory 混合检索：关键词(active) + 向量(Chroma/Ollama)，供 MCP / Hook / viewer 共用"""
...
from add_policy import normalize_category  # noqa: E402

from memory_paths import (  # noqa: E402
    CHROMA_DB_PATH,
    CONFIG_PATH,
    DEFAULT_USER,
    PROJECT_ALIASES_PATH,
    get_chroma_collection as _pool_chroma_collection,
)
...
def get_chroma_client() -> Any:
    """返回共享的 Chroma 客户端（PersistentClient，多进程可共存读写在 SQLite 锁下串行）。"""
...
def detect_project(cwd: str | None = None, *, workspace_aliases: dict[str, str] | None = None) -> str:
    """从工作目录推断 project 标识；workspace aliases 覆盖池级 project_aliases。"""
    ...
    if workspace_aliases:
        from workspace_config import merge_aliases

        aliases = merge_aliases(aliases, workspace_aliases)
    if name in aliases:
        return aliases[name]
...
def merge_and_rank(
    ...
    if read_projects is not None:
        from workspace_config import filter_by_read_access

        merged = filter_by_read_access(merged, read_projects)
    if not project:
