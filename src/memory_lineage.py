...
from memory_paths import HISTORY_DB, LINEAGE_PATH
...
def _history_events(memory_id: str) -> list[dict[str, Any]]:
    ...
    try:
        if conn.execute(
            ...
            rows = conn.execute(
                """
                SELECT event, old_content, new_content, created_at
                FROM memory_events
                WHERE memory_id = ?
    ...
    for event, old_content, new_content, created_at in rows:
        ...
        preview = new_content or old_content or ''
        events.append({
            'ts': created_at or '',
            ...
            'target_id': '',
            'category': '',
            'note': 'memory_events' if action else 'history.db',
