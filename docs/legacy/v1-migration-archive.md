> **归档文档 — 已停用。** 新用户请忽略。沿革摘要见 [history.md](../history.md)。

# 从 mem0-local-enhanced 升级（归档）
...
```bash
cd local-memory
bash scripts/legacy/migrate_full_to_v2.sh
```
...
```bash
python3 scripts/legacy/migrate_history.py --pool ~/.memory/pools/default
python3 scripts/legacy/migrate_config.py --pool ~/.memory/pools/default
python3 scripts/legacy/migrate_chroma_collection.py --pool ~/.memory/pools/default
```
