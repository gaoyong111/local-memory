...
- **无本地降级**：`fallback_llm` 已移除（2026-08-11，本地 qwen2.5:7b 兜底慢/发热且静默）。主 LLM 失败 → 写告警并抛错，调用方（grooming/add_policy）自行跳过或走规则兜底
- **降级告警**：主 LLM 失败时每进程每主端点写一条 pending 告警记忆（`source=llm-degradation-alert`），每日复盘可见——降级不再静默
