...
# 用法：cd local-memory && bash scripts/legacy/migrate_full_to_v2.sh [--dry-run]
set -euo pipefail
...
LEGACY_SCRIPTS="$REPO_ROOT/scripts/legacy"
...
if $DRY_RUN; then
  ...
  "$PY" "$LEGACY_SCRIPTS/migrate_config.py" --pool "$SRC" --dry-run
  "$PY" "$LEGACY_SCRIPTS/migrate_chroma_collection.py" --pool "$SRC" --dry-run
  exit 0
fi
...
# 4. 迁移脚本
"$PY" "$LEGACY_SCRIPTS/migrate_history.py" --pool "$POOL"
echo "[ok] migrate_history"

"$PY" "$LEGACY_SCRIPTS/migrate_config.py" --pool "$POOL"
echo "[ok] migrate_config"

"$PY" "$LEGACY_SCRIPTS/migrate_chroma_collection.py" --pool "$POOL"
